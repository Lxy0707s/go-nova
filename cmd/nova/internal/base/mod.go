...
func KratosMod() string {
	...
	if path, err := ModuleVersion("github.com/go-nova"); err == nil {
		// $GOPATH/pkg/mod/github.com/go-nova/kratos@v2
		return filepath.Join(cachePath, path)
	}
	// $GOPATH/src/github.com/go-nova/kratos
	return filepath.Join(gopath, "src", "github.com", "go-nova", "kratos")
}
