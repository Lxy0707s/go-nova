...
func TestJSON_Marshal(t *testing.T) {
	tests := []struct {
		...
		{
			input:  &testData.TestModel{Id: 1, Name: "go-nova", Hobby: []string{"1", "2"}},
			expect: `{"id":"1","name":"go-nova","hobby":["1","2"],"attrs":{}}`,
		},
		{
...
func TestJSON_Unmarshal(t *testing.T) {
	...
	tests := []struct {
		...
		{
			input:  `{"id":"1","name":"go-nova","hobby":["1","2"],"attrs":{}}`,
			expect: &p2,
		},
		{
			input:  `{"id":1,"name":"go-nova","hobby":["1","2"]}`,
			expect: &p3,
		},
		{
			input:  `"zebra"`,
