...
import (
	...
	kratoserror "github.com/go-nova/pkg/utils/errors"
)
...
func TestBindQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "test",
			args: args{
				vars:   map[string][]string{"name": {"kratos"}, "url": {"https://go-nova.dev/"}},
				target: &TestBind{},
			},
			err:  nil,
			want: &TestBind{"kratos", "https://go-nova.dev/"},
		},
		{
			name: "test1",
			args: args{
				vars:   map[string][]string{"age": {"kratos"}, "url": {"https://go-nova.dev/"}},
				target: &TestBind2{},
			},
		...
		},
		{
			name: "test2",
			args: args{
				vars:   map[string][]string{"age": {"1"}, "url": {"https://go-nova.dev/"}},
				target: &TestBind2{},
			},
			err:  nil,
		...
		},
		{
			...
			args: args{
				req: &http.Request{
					...
					Body:   io.NopCloser(strings.NewReader("name=kratos&url=https://go-nova.dev/")),
				},
				target: &TestBind{},
			},
			err:  nil,
