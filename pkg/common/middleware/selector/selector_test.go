...
func TestFunc(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/go-nova.dev/kratos",
			ctx:  transport.NewServerContext(context.Background(), &Transport{operation: "/go-nova.dev/kratos"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(ctx context.Context, operation string) bool {
				if strings.HasPrefix(operation, "/go-nova.dev") || strings.HasSuffix(operation, "world") {
					return true
				}
				return false
			}).Build()(next)
	...
	tests := []struct {
		...
		},
		{
			...
			ctx: transport.NewServerContext(context.Background(), &Transport{
				...
				headers:   &mockHeader{map[string][]string{"X-Test": {"test2"}, "go-nova": {"kratos"}}},
			}),
		},
		{
			name: "/test.Name/1234",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(ctx context.Context, operation string) bool {
				...
				if tr.RequestHeader().Get("go-nova") == "kratos" {
					return true
				}
				return false
			}).Build()(next)
